...
export default {
    ...
    'tables': {
      handler (val) {
        ...
        if (this.pageto) {
           ...
           // this.pager.total = val.tableData.total
        }
        this.$nextTick(() => {
          this.timeOut()
        })
  ...
  methods: {
    ...
    query () {
    this.pager.pageNum = 1
    var formdata = this.$refs.searchForm.getFormData()
    this.$emit('refreshData', formdata,this.pager);
    ...
    _sizeChange (s) {
      ...
      this.$emit('refreshData', this.$refs.searchForm.getFormData(),this.pager)
    },
    _currentChange (p) {
